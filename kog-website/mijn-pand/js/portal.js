...
import { urgencyAccent, parseYmd, mountWeekCalendar } from '../../admin/js/calendar.js';
...
/* ---------- maintenance calendar (shared widget in ../../admin/js/calendar.js) ---------- */

// Maps this property's upcoming list to the shared calendar's event shape.
function toCalendarEvents(pid, upcoming, today) {
  return upcoming.map(({ component, entry }) => ({
    ...
    line1: componentTypeLabel(component.component_type) + (component.label ? ' — ' + component.label : ''),
    line2: entry.next_maintenance_advice || '',
    href: `#/property/${pid}/component/${component.id}`,
