...
import { STATUS_META } from '../../admin/js/status.js';
...
function statusPill(status, fallback) {
  ...
  const label = meta ? meta.label : (fallback || '—');
...
export async function renderHome(root) {
  root.innerHTML = `<p class="text-ink/40">Laden…</p>`;
  const { data: properties, error } = await supabase.from('properties').select('*').order('name');
  if (error) { root.innerHTML = `<p class="text-sienna2">Er ging iets mis bij het laden. Probeer het later opnieuw.</p>`; return; }

  if (!properties || !properties.length) {
    root.innerHTML = `
      <div class="bg-white border border-rule rounded-2xl p-10 text-center">
        <h1 class="text-[20px] font-semibold">Nog geen pand gekoppeld</h1>
        <p class="mt-2 text-[14px] text-ink/55">Uw account is nog niet aan een pand gekoppeld. Neem contact op met KOG.</p>
      </div>`;
    return;
  }
  ...
  root.innerHTML = `
    <h1 class="text-[24px] font-semibold mb-1">Uw panden</h1>
    <p class="text-[14px] text-ink/55 mb-6">Kies een pand om het onderhoudsdossier te bekijken.</p>
  ...
  properties.forEach(p => {
    ...
    card.innerHTML = `
      <div class="text-[11px] uppercase tracking-[.18em] text-leaf2 mb-2">${PROPERTY_TYPE_LABEL[p.type] || ''}</div>
...
function heroCard(components) {
  ...
  if (!next) {
    return `
      <div class="rounded-2xl p-7 border border-rule bg-white">
        <div class="text-[11px] uppercase tracking-[.18em] text-ink/45 mb-2">Volgende onderhoud</div>
        <div class="text-[19px] font-semibold" style="color:#4C8B30;">Geen onderhoud gepland</div>
        <p class="mt-1.5 text-[13.5px] text-ink/55">Zodra KOG werk logt met een volgende inspectiedatum, ziet u die hier.</p>
      </div>`;
  }
  ...
  if (days < 0) { accentBg = 'rgba(206,27,36,.07)'; accentBorder = 'rgba(206,27,36,.35)'; accentFg = '#A3141B'; note = 'Deze datum is verstreken — KOG neemt contact met u op.'; }
  else if (days <= 90) { accentBg = '#FDF3D8'; accentBorder = '#EAD9A0'; accentFg = '#8A6D1D'; note = 'Binnenkort — KOG neemt tegen die tijd contact met u op.'; }
  return `
...
export async function renderProperty(root, id) {
  ...
  if (!property) { root.innerHTML = `<p class="text-sienna2">Pand niet gevonden.</p>`; return; }

  const isVve = property.type === 'vve';
  root.innerHTML = `
    <div class="mb-7">
      <div class="text-[11px] uppercase tracking-[.18em] text-leaf2 mb-2">${PROPERTY_TYPE_LABEL[property.type] || ''}</div>
      ...
      ${isVve ? `<a href="#/property/${id}/rapport" class="no-print inline-flex items-center gap-2 mt-4 px-4 py-2.5 border border-rule rounded-lg text-[13.5px] hover:border-sienna transition-colors bg-white">
        ...
        Bestuursrapport bekijken
      </a>` : ''}
    </div>
    ${heroCard(components)}
    <h2 class="mt-9 mb-4 text-[11px] uppercase tracking-[.18em] text-ink/50">Bouwdelen</h2>
    <div id="components-grid" class="grid gap-3 sm:grid-cols-2"></div>
  `;

  const grid = root.querySelector('#components-grid');
  if (!components.length) {
    grid.innerHTML = `<p class="text-[14px] text-ink/50">Nog geen bouwdelen vastgelegd voor dit pand.</p>`;
    return;
  }
  components.forEach(c => {
    ...
    card.innerHTML = `
      <div class="flex items-start justify-between gap-3">
        ...
        ${statusPill(latest?.status, 'Nog geen werk gelogd')}
      </div>
      ${nextDate ? `<div class="mt-3 text-[13px] text-ink/55">Volgende inspectie: ${fmtDate(nextDate)}</div>` : ''}
      <div class="mt-1 text-[13px] text-ink/40">${(c.maintenance_entries || []).length} ${(c.maintenance_entries || []).length === 1 ? 'registratie' : 'registraties'} →</div>
    `;
    grid.appendChild(card);
  });
}
...
export async function renderComponent(root, pid, cid) {
  ...
  if (!component) { root.innerHTML = `<p class="text-sienna2">Bouwdeel niet gevonden.</p>`; return; }
  ...
  if (!entries || !entries.length) {
    timeline.innerHTML = `<p class="text-[14px] text-ink/50">Nog geen werk gelogd voor dit bouwdeel.</p>`;
    return;
  }

  const PHASE_LABEL = { before: 'Vóór', during: 'Tijdens', after: 'Ná' };
  for (const e of entries) {
    const card = document.createElement('div');
    card.className = 'bg-white border border-rule rounded-2xl p-6';
    card.innerHTML = `
      <div class="flex items-center justify-between flex-wrap gap-2">
        <div class="text-[15px] font-semibold">${e.date_carried_out ? fmtDate(e.date_carried_out) : 'Gepland'}</div>
        ${statusPill(e.status)}
      </div>
      ${e.work_done ? `<p class="mt-3 text-[14.5px] leading-relaxed">${escapeHtml(e.work_done)}</p>` : ''}
      <dl class="mt-3 space-y-1.5 text-[13.5px]">
        ${e.findings ? `<div><dt class="inline text-ink/45">Bevindingen: </dt><dd class="inline">${escapeHtml(e.findings)}</dd></div>` : ''}
        ${e.materials_used ? `<div><dt class="inline text-ink/45">Materialen: </dt><dd class="inline">${escapeHtml(e.materials_used)}</dd></div>` : ''}
        ${e.handover_notes ? `<div><dt class="inline text-ink/45">Oplevering: </dt><dd class="inline">${escapeHtml(e.handover_notes)}</dd></div>` : ''}
        ${e.next_inspection_date ? `<div><dt class="inline text-ink/45">Volgende inspectie: </dt><dd class="inline font-semibold">${fmtDate(e.next_inspection_date)}</dd></div>` : ''}
      </dl>
    ...
    if (photos.length) {
      ...
      for (const phase of ['before', 'during', 'after']) {
        ...
        groupEl.innerHTML = `<div class="text-[11px] uppercase tracking-[.15em] text-ink/45 mb-1.5">${PHASE_LABEL[phase]}</div><div class="flex flex-wrap gap-2"></div>`;
        const thumbs = groupEl.querySelector('.flex');
        group.forEach(p => {
          const a = document.createElement('a');
          a.href = p.url; a.target = '_blank'; a.rel = 'noopener';
          a.innerHTML = `<img src="${p.url}" class="w-24 h-24 object-cover rounded-lg border border-rule hover:opacity-90" alt="Foto ${PHASE_LABEL[phase]}" />`;
