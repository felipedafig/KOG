...
function statusPill(status) {
  ...
  if (!meta) return `<span class="inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold" style="background:#F1F0EA;color:#6B6862">Nog geen werk</span>`;
  return `<span class="inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold" style="background:${meta.bg};color:${meta.fg}">${meta.label}</span>`;
}
...
export async function renderRapport(root, id) {
  ...
  const summaryTiles = statusOrder.filter(s => counts[s]).map(s => {
    const meta = STATUS_META[s] || { label: 'Nog geen werk', bg: '#F1F0EA', fg: '#6B6862' };
    return `<div class="rapport-card rounded-xl p-4 border border-rule bg-white text-center">
      ...
      <div class="text-[11.5px] uppercase tracking-wide text-ink/50 mt-0.5">${meta.label}</div>
    </div>`;
  }).join('');

  root.innerHTML = `
    <a href="#/property/${id}" class="no-print text-[13px] text-ink/50 hover:text-sienna">&larr; Terug naar overzicht</a>

    <div class="flex items-start justify-between mt-3 mb-2 flex-wrap gap-3">
      <div>
        <div class="text-[11px] uppercase tracking-[.18em] text-leaf2 mb-1">Bestuursrapport · ${fmtDate(now)}</div>
    ...
    <p class="text-[13px] text-ink/45 mb-7">Opgesteld door Kortlevers Onderhoud Groep — overzicht van uitgevoerd en gepland onderhoud per bouwdeel.</p>

    <h2 class="text-[11px] uppercase tracking-[.18em] text-ink/50 mb-3">Stand van zaken</h2>
    <div class="grid gap-3 mb-9" style="grid-template-columns:repeat(auto-fit,minmax(120px,1fr));">${summaryTiles || '<p class="text-[14px] text-ink/50">Nog geen bouwdelen vastgelegd.</p>'}</div>

    <h2 class="text-[11px] uppercase tracking-[.18em] text-ink/50 mb-3">Per bouwdeel</h2>
      ...
      <table class="w-full text-[13.5px]">
        <thead>
          <tr class="border-b border-rule text-left">
            <th class="px-5 py-3 font-semibold">Bouwdeel</th>
            <th class="px-5 py-3 font-semibold">Status</th>
            <th class="px-5 py-3 font-semibold">Laatste werk</th>
            <th class="px-5 py-3 font-semibold">Volgende inspectie</th>
          </tr>
        </thead>
        <tbody>
          ${components.map(c => {
          ...
          }).join('') || '<tr><td class="px-5 py-4 text-ink/50" colspan="4">Nog geen bouwdelen.</td></tr>'}
        </tbody>
      </table>
    </div>

    <h2 class="text-[11px] uppercase tracking-[.18em] text-ink/50 mb-3">Uitgevoerd in ${thisYear}</h2>
    <div class="space-y-2.5 mb-9">
      ${doneThisYear.map(({ c, e }) => `
        ...
        </div>`).join('') || `<p class="text-[14px] text-ink/50">Geen afgerond werk geregistreerd in ${thisYear}.</p>`}
    </div>

    <h2 class="text-[11px] uppercase tracking-[.18em] text-ink/50 mb-3">Komend onderhoud</h2>
    <div class="space-y-2.5">
      ${upcoming.map(({ c, e }) => `
        ...
        </div>`).join('') || '<p class="text-[14px] text-ink/50">Nog geen gepland onderhoud.</p>'}
    </div>
  `;
}
